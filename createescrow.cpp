...
void create_escrow::define(name &owner, string dapp, uint64_t ram_bytes, asset net, asset cpu, uint64_t pricekey, airdropdata &airdrop, bool use_rex, rexdata &rex)
{
    ...
    check(iterator == dapps.end() || (iterator != dapps.end() && iterator->owner == owner),
          ("the dapp " + dapp + " is already registered by another account [escrow.define]").c_str());

    uint64_t min_ram = create_escrow::getMinimumRAM();

    check(ram_bytes >= min_ram, ("ram for new accounts must be equal to or greater than " + to_string(min_ram) + " bytes. [escrow.define]").c_str());
...
void create_escrow::whitelist(name owner, name account, string dapp)
{
    ...
    if (iterator != dapps.end() && owner == iterator->owner)
        ...
        check(false, ("the dapp " + dapp + " is not owned by account " + owner.to_string() + "[escrow.whitelist]").c_str());
}
    ...
    if (sym == create_escrow::getCoreSymbol().code().to_string())
    {
        ...
        if (iterator != balances.end())
        {
            ...
            balances.modify(iterator, same_payer, [&](auto &row) {
                ...
                {
                    check(false, ("no remaining contribution for " + dapp + " by " + reclaimer.to_string() + " [escrow.reclaim]").c_str());
                }
        ...
        {
            check(false, ("no funds given by " + reclaimer.to_string() + " for " + dapp + " [escrow.reclaim]").c_str());
        }
    }
        ...
        if (iterator != dapps.end())
            dapps.modify(iterator, same_payer, [&](auto &row) {
                if (row.airdrop->contract != name("") && row.airdrop->balance.symbol.code().to_string() == sym && row.owner == name(reclaimer))
                {
                    ...
                    {
                        check(false, ("No remaining airdrop balance for " + dapp + ". [escrow.reclaim]").c_str());
                    }
                }
                else
                {
                    check(false, ("the remaining airdrop balance for " + dapp + " can only be claimed by its owner/whitelisted account. [escrow.reclaim]").c_str());
                }
            });
    }
}
...
bool create_escrow::checkIfOwnerOrWhitelisted(name account, string origin)
{
    ...
    if (iterator != dapps.end())
    {
        if (account == iterator->owner)
            ...
            check(false, ("only owner or whitelisted accounts can call this action for " + origin + "[escrow.checkIfOwnerOrWhitelisted]").c_str());
    }
    else
    {
        check(false, ("no owner account found for " + origin + "[escrow.checkIfOwnerOrWhitelisted]").c_str());
    }
}
